import pytest
import os.path
from openserver.openserver import OpenServer, DoGet, DoSet, DoCmd
...
def test_openserver_functions():
    ...
    c.DoCmd("GAP.START()")
    c.DoCmd(f'GAP.OPENFILE("{gapfile}")')
...
def test_get_full_array():
    assert np.array_equal(
        c.DoGet("PROSPER.SIN.EQP.Devn.Data[$].Md"),
        np.array([0.0, 100.0, 1000.0, 2000.0]),
    )


def test_get_parts_array():
    assert np.array_equal(
        c.DoGet("PROSPER.SIN.EQP.Devn.Data[1:3].Md"), np.array([100.0, 1000.0, 2000.0])
    )


def test_get_selection_array():
    assert np.array_equal(
        c.DoGet("PROSPER.SIN.EQP.Devn.Data[0,2].Tvd"), np.array([0.0, 300.0])
    )


def test_get_equipment_array():
    assert np.array_equal(
        c.DoGet("GAP.MOD[0].WELL[{@testWell#}].Label"),
        np.array(["testWell1", "testWell2", "testWell3", "testWell4"]),
    )
...
def test_product_prefix():
    with pytest.raises(
        ValueError, match="The tag string product prefix was not recognised"
    ):
        c.DoCmd("Excel.value")
    with pytest.raises(
        ValueError, match="The tag string product prefix was not recognised"
